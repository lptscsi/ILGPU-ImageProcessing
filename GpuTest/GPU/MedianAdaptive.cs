...
{
    /// <summary>
    /// Median Adaptive (splits input into individiual channels, and uses adaptive methodology)
    /// </summary>
    public static class MedianAdaptive
    {
        ...
        public struct Config
        {
            ...
            public readonly ArrayView2D<byte, Stride2D.DenseX> mask;

            public Config(int width, int height, ArrayView2D<float, Stride2D.DenseX> input, ArrayView2D<float, Stride2D.DenseX> output, ArrayView2D<byte, Stride2D.DenseX> mask)
            {
                this.width = width;
                this.height = height;
                this.input = input;
        ...
        public static async Task<GPUImage<RGBA32>> Execute(Accelerator device, GPUImage<RGBA32> inputImage)
        {
            ...
            using MemoryBuffer2D<byte, Stride2D.DenseX> bufferMaskR = device.Allocate2DDenseX<byte>(new LongIndex2D(inputImage.Extent2D.X, inputImage.Extent2D.Y));
            using MemoryBuffer2D<byte, Stride2D.DenseX> bufferMaskG = device.Allocate2DDenseX<byte>(new LongIndex2D(inputImage.Extent2D.X, inputImage.Extent2D.Y));
            using MemoryBuffer2D<byte, Stride2D.DenseX> bufferMaskB = device.Allocate2DDenseX<byte>(new LongIndex2D(inputImage.Extent2D.X, inputImage.Extent2D.Y));
            bufferMaskR.MemSetToZero();
            bufferMaskG.MemSetToZero();
            bufferMaskB.MemSetToZero();
            ...
            var confR = new Config(inputImage.Extent2D.X, inputImage.Extent2D.Y, bufferR.View, buffer2R.View, bufferMaskR.View);
            var confG = new Config(inputImage.Extent2D.X, inputImage.Extent2D.Y, bufferG.View, buffer2G.View, bufferMaskG.View);
            var confB = new Config(inputImage.Extent2D.X, inputImage.Extent2D.Y, bufferB.View, buffer2B.View, bufferMaskB.View);
            ...
            Action<AcceleratorStream, KernelConfig, Config> kernel1 = LoadKernel1(device);
            Action<AcceleratorStream, KernelConfig, Config> kernel2 = LoadKernel2(device);
            Action<AcceleratorStream, KernelConfig, Config> kernel3 = LoadKernel3(device);

            forkKernel(stream, kernelConfig, forkConf);
            device.Synchronize();

            kernel1(stream, kernelConfig, confR);
            kernel1(stream, kernelConfig, confG);
            kernel1(stream, kernelConfig, confB);
            device.Synchronize();
           
            kernel2(stream, kernelConfig, confR);
            kernel2(stream, kernelConfig, confG);
            kernel2(stream, kernelConfig, confB);
            device.Synchronize();

            kernel3(stream, kernelConfig, confR);
            kernel3(stream, kernelConfig, confG);
            kernel3(stream, kernelConfig, confB);
            device.Synchronize();
        ...
        static void Kernel1(Config conf)
        {
            ...
            float zxy = conf.input[index];
            conf.output[index] = zxy;
            ...
            float min = MinMax.min9(arr);
            float max = MinMax.max9(arr);
            float med = Median.median9(arr);
        ...
        static void Kernel2(Config conf)
        {
            ...
            float min = MinMax.min25(arr);
            float max = MinMax.max25(arr);
            float med = Median25.median(arr);
        ...
        static void Kernel3(Config conf)
        {
            ...
            float min = MinMax.min49(arr);
            float max = MinMax.max49(arr);
            float med = Median49.median(arr);
